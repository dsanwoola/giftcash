...
export async function GET(req: Request) {
  try {
    ...
    const db = adminDb();
    let query: Query = db.collection("bank_alerts").orderBy("createdAt", "desc").limit(100);
    if (status !== "all") query = db.collection("bank_alerts").where("status", "==", status).limit(100);
    ...
    await Promise.all(refs.map(async (ref) => {
      const intentSnap = await db.collection("payment_intents").doc(ref).get();
